...
debug$1('required');
class EditorInvisible extends paper$1.PaperScope {
  constructor() {
    super();
    this._undo = {
      clear() {},
...
class Contour extends AbstractFilling(paper$1.Layer) {
  ...
  glass_nodes(path, nodes, bind) {
    ...
    let curve, findedb, d, node1, node2;
    if (!nodes) {
      nodes = this.nodes;
    }
...
class DimensionLineCustom extends DimensionLine {
  ...
  get path() {
    if(this.fix_angle) {
      ...
      const t = d.clone();
      t.angle = this.angle;
      const path = new paper$1.Path({insert: false, segments: [b, b.add(t)]});
...
class DimensionRadius extends DimensionLineCustom {
  ...
  get path() {
    ...
    const res = new paper$1.Path({insert: false, segments: [b, b.add(n)]});
    res.offset = 0;
    return res;
  }
  redraw() {
...
class Scheme extends paper$1.Project {
  ...
  check_distance(element, profile, res, point, check_only) {
    ...
    let distance, gp, cnns, bind_node = typeof check_only == 'string' && check_only.indexOf('node') != -1,
