/*!
 windowbuilder-reports v2.0.237, built:2018-01-29
...
var paper$1 = _interopDefault(require('paper/dist/paper-core.js'));
...
var chartscharacteristics = function ($p) {
	cch_predefined_elmnts($p);
	cch_properties($p);
};

var i18ru = function ($p) {
  Object.assign($p.msg, {
...
var common = function ($p) {
  i18ru($p);
};

var modifiersNew = function ($p) {
...
var meta_pouchdb = function(proto) {
...
const debug$2 = require('debug')('wb:meta');
...
(async () => {
  ...
  modifiers($p);
  modifiersNew($p);
  debug$2('inited & modified');
  const {pouch} = $p.adapters;
  pouch.log_in(user_node.username, user_node.password)
    .then(() => pouch.load_data())
    .catch((err) => debug$2(err));
  pouch.on({
    user_log_in(name) {
      debug$2(`logged in ${$p.job_prm.couch_local}, user:${name}, zone:${$p.job_prm.zone}`);
    },
    user_log_fault(err) {
      debug$2(`login error ${err}`);
    },
    pouch_load_start(page) {
      debug$2('loadind to ram: start');
    },
    pouch_data_page(page) {
      debug$2(`loadind to ram: page №${page.page} (${page.page * page.limit} from ${page.total_rows})`);
    },
    pouch_complete_loaded(page) {
      debug$2(`ready to receive queries, listen on port: ${process.env.PORT || 3000}`);
    },
    pouch_doc_ram_loaded() {
      pouch.local.ram.changes({
        ...
        debug$2(`change error ${err}`);
      });
      debug$2(`loadind to ram: READY`);
...
debug$1('required');
class Editor extends paper$1.PaperScope {
...
const AbstractFilling = (superclass) => class extends superclass {
  is_pos(pos) {
    ...
    if(!res){
      let rect;
      if(pos == "top"){
        rect = new paper$1.Rectangle(this.bounds.topLeft, this.bounds.topRight.add([0, -200]));
      }
      else if(pos == "left"){
        rect = new paper$1.Rectangle(this.bounds.topLeft, this.bounds.bottomLeft.add([-200, 0]));
      }
      else if(pos == "right"){
        rect = new paper$1.Rectangle(this.bounds.topRight, this.bounds.bottomRight.add([200, 0]));
      }
      else if(pos == "bottom"){
        rect = new paper$1.Rectangle(this.bounds.bottomLeft, this.bounds.bottomRight.add([0, 200]));
      }
...
class GlassSegment {
  ...
  get _sub() {
    ...
    return {
      get b() {
        return sub_path ? sub_path.firstSegment.point : new paper$1.Point();
      },
      set b(v) {
        sub_path && (sub_path.firstSegment.point = v);
      },
      get e() {
        return sub_path ? sub_path.lastSegment.point : new paper$1.Point();
      },
      set e(v) {
...
class Contour extends AbstractFilling(paper$1.Layer) {
  constructor(attr) {
    ...
    glass_contours.forEach((glcontour) => {
      ...
      for (let g in glasses) {
        ...
        if (crating == rating && cglass != glass) {
          if (!glass_center) {
            glass_center = glcontour.reduce((sum, val) => sum.add(val.b), new paper$1.Point).divide(glcontour.length);
          }
  ...
  get bounds() {
    ...
    if (!_attr._bounds || !_attr._bounds.width || !_attr._bounds.height) {
      ...
      if (!_attr._bounds) {
        _attr._bounds = new paper$1.Rectangle();
      }
    }
  ...
  get dimension_bounds() {
    ...
    if (ib.height && ib.bottom > bounds.bottom) {
      ...
      bounds = bounds.unite(
        new paper$1.Rectangle(bounds.bottomLeft, bounds.bottomRight.add([0, delta < 250 ? delta * 1.1 : delta * 1.2]))
      );
    }
    return bounds;
  }
  ...
  draw_cnn_errors() {
    ...
    if (l_visualization._cnn) {
      ...
      l_visualization._cnn = new paper$1.Group({parent: l_visualization});
    }
    const err_attrs = {
      strokeColor: 'red',
      strokeWidth: 2,
  ...
  draw_mosquito() {
    ...
    this.project.ox.inserts.find_rows({cnstr: this.cnstr}, (row) => {
      if (row.inset.insert_type == $p.enm.inserts_types.МоскитнаяСетка) {
        const props = {
          parent: new paper$1.Group({parent: l_visualization._by_insets}),
          strokeColor: 'grey',
          strokeWidth: 3,
          dashArray: [6, 4],
          strokeScaling: false,
        ...
        if (imposts) {
          ...
          function add_impost(y) {
            const impost = Object.assign(new paper$1.Path({
              insert: false,
  ...
  draw_opening() {
    ...
    if (!l_visualization._opening) {
      l_visualization._opening = new paper$1.CompoundPath({
  ...
  bounds_inner(size) {
    const path = new paper$1.Path({insert: false});
  ...
  get l_text() {
    const {_attr} = this;
    return _attr._txt || (_attr._txt = new paper$1.Group({parent: this}));
  }
  get l_visualization() {
    const {_attr} = this;
    if (!_attr._visl) {
      _attr._visl = new paper$1.Group({parent: this, guide: true});
      _attr._visl._by_insets = new paper$1.Group({parent: _attr._visl});
      _attr._visl._by_spec = new paper$1.Group({parent: _attr._visl});
    }
    return _attr._visl;
  }
  get opacity() {
...
class DimensionLayer extends paper$1.Layer {
  get bounds() {
...
class DimensionDrawer extends paper$1.Group {
  constructor(attr) {
    super(attr);
    this.bringToFront();
  }
...
class DimensionLine extends paper$1.Group {
  constructor(attr) {
    ...
    new paper$1.Path({parent: this, name: 'callout1', strokeColor: 'black', guide: true});
    new paper$1.Path({parent: this, name: 'callout2', strokeColor: 'black', guide: true});
    new paper$1.Path({parent: this, name: 'scale', strokeColor: 'black', guide: true});
    new paper$1.PointText({
      parent: this,
      name: 'text',
  ...
  _mouseenter() {
    paper$1.canvas_cursor('cursor-arrow-ruler');
  }
  _mouseleave() {
  }
  _click(event) {
  ...
  _move_points(event, xy) {
    ...
    if(_attr.elm1){
      ...
      if(this.pos == "top" || this.pos == "bottom"){
        ...
        if(event.name == "right"){
          delta = new paper$1.Point(event.size - size, 0);
          _bounds[event.name] = Math.max(p1.x, p2.x);
        }
        else{
          delta = new paper$1.Point(size - event.size, 0);
        ...
        if(event.name == "bottom"){
          delta = new paper$1.Point(0, event.size - size);
          _bounds[event.name] = Math.max(p1.y, p2.y);
        }
        else{
          delta = new paper$1.Point(0, size - event.size);
      ...
      if(this.pos == "top" || this.pos == "bottom")
        if(event.name == "right")
          delta = new paper$1.Point(event.size - _bounds.width, 0);
        else
          delta = new paper$1.Point(_bounds.width - event.size, 0);
      else{
        if(event.name == "bottom")
          delta = new paper$1.Point(0, event.size - _bounds.height);
        else
          delta = new paper$1.Point(0, _bounds.height - event.size);
      }
    }
    if(delta.length){
      const {project} = this;
...
class DimensionLineCustom extends DimensionLine {
  ...
  _click(event) {
    event.stop();
    if(paper$1.tool instanceof ToolRuler){
      this.selected = true;
    }
  }
  _mouseenter() {
    if(paper$1.tool instanceof ToolRuler){
      ...
      paper$1.canvas_cursor('cursor-arrow-ruler-dis');
    }
  }
}
class BuilderElement extends paper$1.Group {
  constructor(attr) {
    super(attr);
    if(!attr.row){
  ...
  set_clr(v, ignore_select) {
    this._row.clr = v;
    if(this.path instanceof paper$1.Path){
...
class Filling extends AbstractFilling(BuilderElement) {
  ...
  initialize(attr) {
    ...
    if(_row.path_data){
      _attr.path = new paper$1.Path(_row.path_data);
    }
    else if(attr.path){
      _attr.path = new paper$1.Path();
      this.path = attr.path;
    }
    else{
      _attr.path = new paper$1.Path([
  ...
  fill_error() {
    const {path} = this;
    path.fillColor = new paper$1.Color({
      stops: ["#fee", "#fcc", "#fdd"],
  ...
  set path(attr) {
    ...
    if(path){
      ...
      path = _attr.path = new paper$1.Path({parent: this});
    }
  ...
  get bounds() {
    const {path} = this;
    return path ? path.bounds : new paper$1.Rectangle();
  }
  get x1() {
...
class FreeText extends paper$1.PointText {
  constructor(attr) {
    if(!attr.fontSize){
    ...
    if(attr.point){
      if(attr.point instanceof paper$1.Point)
        this.point = attr.point;
      else
        this.point = new paper$1.Point(attr.point);
    }
    else{
      this.clr = _row.clr;
  ...
  remove() {
    this._row._owner.del(this._row);
    this._row = null;
    paper$1.PointText.prototype.remove.call(this);
  }
  save_coordinates() {
    const {_row} = this;
    _row.x1 = this.x;
...
class GeneratrixElement extends BuilderElement {
  ...
  set x1(v) {
    ...
    if(bounds && (v = parseFloat(v) + bounds.x - this.b.x)){
      this.select_node("b");
      this.move_points(new paper$1.Point(v, 0));
    }
  }
  get y1() {
    const {bounds} = this.project;
  ...
  set y1(v) {
    ...
    if(bounds && (v = bounds.height + bounds.y - parseFloat(v) - this.b.y)){
      this.select_node("b");
      this.move_points(new paper$1.Point(0, v));
    }
  }
  get x2() {
    const {bounds} = this.project;
    ...
    this.generatrix.segments.forEach((segm) => {
      ...
      if (segm.selected || all_points){
        ...
        if(segm.point == this.b){
          ...
          if(!cnn_point.profile_point || paper$1.Key.isDown('control')){
        ...
        if(cnn_point && cnn_point.cnn_types == $p.enm.cnn_types.acn.t && (segm.point == this.b || segm.point == this.e)){
          ...
          segm.point = free_point;
          if(cnn_point && !paper$1.Key.isDown('control')){
  ...
  arc_r: {
	...
	snap_to_angle: {
        ...
        snapAngle = Math.PI*2/8;
			...
			return new paper$1.Point(dirx*d, diry*d);
		}
	},
  bind_to_nodes: {
...
class CnnPoint {
  ...
  check_err(style) {
    ...
    if(!cnn || (cnn.lmin > 0 && cnn.lmin > len) || (cnn.lmax > 0 && cnn.lmax < len)) {
      if(style) {
        Object.assign(new paper$1.Path.Circle({
...
class ProfileRays {
  constructor(parent) {
    ...
    this.inner = new paper$1.Path({insert: false});
    this.outer = new paper$1.Path({insert: false});
  }
  clear_segments() {
...
class ProfileItem extends GeneratrixElement {
  ...
  get arc_h() {
    ...
    if(_row.r) {
      ...
      return paper$1.Line.getSignedDistance(b.x, b.y, e.x, e.y, p.x, p.y).round(1);
    }
    return 0;
  }
  set arc_h(v) {
  ...
  get angle_hor() {
    const {b, e} = this;
    const res = (new paper$1.Point(e.x - b.x, b.y - e.y)).angle.round(2);
  ...
  setSelection(selection) {
    ...
    if(selection) {
      ...
      if(this._hatching) {
        ...
        this._hatching = new paper$1.CompoundPath({
          parent: this,
          guide: true,
          strokeColor: 'grey',
          strokeScaling: false
  ...
  initialize(attr) {
    ...
    if(attr.generatrix) {
      ...
      if(_row.path_data) {
        _attr.generatrix = new paper$1.Path(_row.path_data);
      }
      else {
        const first_point = new paper$1.Point([_row.x1, h - _row.y1]);
  ...
  set_generatrix_radius(height) {
    ...
    if(_row.r) {
      let p = new paper$1.Point(b.arc_point(b.x, b.y, e.x, e.y, _row.r, _row.arc_ccw, false));
      if(p.point_pos(b.x, b.y, e.x, e.y) > 0 && !_row.arc_ccw || p.point_pos(b.x, b.y, e.x, e.y) < 0 && _row.arc_ccw) {
        p = new paper$1.Point(b.arc_point(b.x, b.y, e.x, e.y, _row.r, !_row.arc_ccw, false));
      }
      if(full || height) {
...
class ProfileConnective extends ProfileItem {
  ...
  move_points(delta, all_points, start_point) {
    ...
    if(all_points !== false && !paper$1.Key.isDown('control')){
      nearests.forEach((np) => {
...
class ConnectiveLayer extends paper$1.Layer {
  redraw() {
...
class Scheme extends paper$1.Project {
            ...
            .then(() => {
              if(_scheme.ox.coordinates.count()) {
                ...
                if(from_service){
                  ...
                  paper$1.load_stamp && paper$1.load_stamp();
                }
              }
  ...
  get strokeBounds() {
    let bounds = new paper$1.Rectangle();
  ...
  deselect_all_points(with_items) {
    this.getItems({class: paper$1.Path}).forEach((item) => {
      item.segments.forEach((segm) => {
        if(segm.selected) {
          segm.selected = false;
        }
...
class Sectional extends GeneratrixElement {
  ...
  draw_angle(ind) {
    ...
    children.push(new paper$1.Path.Arc({
      from,
...
class ProductsBuilding$1 {
  constructor(listen) {
    ...
    function cnn_add_spec(cnn, elm, len_angl, cnn_other) {
      ...
      const {new_spec_row, calc_count_area_mass} = ProductsBuilding$1;
    ...
    function cnn_filter_spec(cnn, elm, len_angl) {
      ...
      const {check_params} = ProductsBuilding$1;
    ...
    function furn_check_opening_restrictions(contour, cache) {
      let ok = true;
      const {new_spec_row} = ProductsBuilding$1;
...
if(typeof global !== 'undefined'){
  global.ProductsBuilding = ProductsBuilding$1;
}
$p.ProductsBuilding = ProductsBuilding$1;
$p.products_building = new ProductsBuilding$1(true);
